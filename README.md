...
Для описаний модов редактор поддерживает предварительный машинный перевод через LibreTranslate-совместимый API. Исходный и целевой языки выбираются отдельно, результат сначала открывается для ручной проверки и только затем сохраняется. Адрес сервиса и необязательный API-ключ хранятся локально в `local/coolpackhelper/author-settings.json`; во внешний сервис отправляется только выбранное описание.
...
- создавать черновой перевод описания на любой язык через настраиваемый LibreTranslate-совместимый сервис;
