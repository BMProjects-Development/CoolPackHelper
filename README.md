# CoolPackHelper

Клиентский помощник для сборок Minecraft. При запуске он проверяет настроенные моды, их версии и показывает адаптивный список со ссылками на скачивание.

Версия платформы: Minecraft 1.21.1, NeoForge 21.1.x, Java 21.

## Конфигурация

При первом запуске создаётся `config/coolpackhelper.json`. Демонстрационные записи отключены (`"enabled": false`), поэтому меню не появится, пока автор сборки не включит и не настроит хотя бы один мод.

Основная структура:

```json
{
  "schemaVersion": 2,
  "pack": {
    "id": "my-pack",
    "name": "My Modpack",
    "version": "1.0.0"
  },
  "showPolicy": "UNTIL_RESOLVED",
  "menu": {
    "defaultLanguage": "ru_ru",
    "translations": {
      "ru_ru": {
        "title": "Требования сборки",
        "description": "Некоторые моды отсутствуют или имеют неподдерживаемую версию."
      }
    }
  },
  "mods": [
    {
      "enabled": true,
      "category": "REQUIRED",
      "name": "FTB Quests",
      "modId": "ftbquests",
      "versionRange": "[2101.1.0,)",
      "filePattern": "ftb-quests-*.jar",
      "descriptions": {
        "ru_ru": "Добавляет книгу заданий этой сборки.",
        "en_us": "Adds the quest book used by this modpack."
      },
      "links": [
        {
          "label": "CurseForge",
          "url": "https://www.curseforge.com/minecraft/mc-mods/ftb-quests-forge"
        },
        {
          "label": "Modrinth",
          "url": "https://modrinth.com/mod/ftb-quests"
        }
      ]
    }
  ]
}
```

### Обнаружение и версии

- `modId` проверяет действительно загруженный NeoForge-мод и является предпочтительным способом.
- `versionRange` использует синтаксис Maven: `[1.0,2.0)`, `[1.5,)`, `(,3.0]` и так далее.
- `filePattern` поддерживает маски `*` и `?` без учёта регистра. Она используется как самостоятельный запасной детектор только для записей без `modId`.
- Если заполнен `modId`, наличие похожего JAR не скрывает ошибку загрузки или неверную версию.

### Категории

- `REQUIRED` — обязательный мод, карточка отмечается красным цветом.
- `RECOMMENDED` — рекомендуемый мод, карточка отмечается жёлтым цветом.

Обе категории показываются в меню, но пользователь всегда может продолжить запуск. CoolPackHelper является помощником, а не загрузчиком: если другой мод объявил отсутствующую зависимость обязательной на уровне NeoForge, загрузчик остановит игру до появления главного меню.

### Политика показа

Поле `showPolicy` принимает:

- `UNTIL_RESOLVED` — показывать при каждом запуске, пока требования не выполнены;
- `ONCE_PER_PACK_VERSION` — один раз для сочетания `pack.id` и `pack.version`;
- `ONCE_EVER` — один раз за всё время;
- `NEVER` — не показывать меню.

Состояние хранится отдельно в `config/coolpackhelper-state.json`. Старые поля `showOnlyOnce`, `requiredMods` и `downloadUrl` схемы v1 продолжают поддерживаться.

### Ссылки и описания

В `links` можно добавить любое количество HTTP/HTTPS-источников. При наличии нескольких источников игрок увидит отдельный экран выбора. Minecraft попросит подтверждение перед открытием ссылки.

`descriptions` содержит комментарии о назначении мода для каждого языка. Порядок выбора текста: точный язык Minecraft, общий код языка (`pt_br` → `pt`), `menu.defaultLanguage`, затем `en_us`. Для простого нелокализованного описания можно использовать поле `description`.

### Интерфейс и проверка конфига

- список модов прокручивается;
- карточки, заголовок и нижние кнопки перестраиваются для узких экранов и крупного GUI Scale;
- длинные названия, статусы и описания ограничиваются доступной шириной;
- доступны кнопки повторной проверки и открытия папки `mods`;
- ошибки конфига показываются на отдельном экране с путём проблемного поля;
- после редактирования файла можно нажать «Проверить снова» без перезапуска игры.

Все подписи интерфейса находятся в `menu.translations`. Полный набор полей можно увидеть в автоматически созданном конфиге. Поддерживаются плейсхолдеры `{required}`, `{recommended}`, `{installed}`, `{current}`, `{total}`, `{count}` и `{mod}` в соответствующих строках.

## Сборка

```powershell
.\gradlew.bat build
```

Готовый JAR появится в `build/libs`.
