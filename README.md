...
`iconUrl`, `projectLinks`, `authors` и `license` содержат сохранённый снимок метаданных проекта. В `projectLinks` отдельно хранятся страница проекта, исходный код, баг-трекер, wiki, Discord и ссылки поддержки; игрок может открыть их из экрана подробностей. Иконка загружается только по HTTPS с официальных CDN Modrinth/CurseForge, ограничивается размером 1 MiB и разрешением 1024×1024. Ошибка загрузки иконки не мешает работе меню.
