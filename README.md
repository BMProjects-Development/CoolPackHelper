...
Для описаний модов редактор поддерживает предварительный машинный перевод через LibreTranslate, DeepL Free/Pro и Google Cloud Translation Basic. Исходный и целевой языки выбираются отдельно, результат сначала открывается для ручной проверки и только затем сохраняется. Для LibreTranslate можно указать собственный адрес; DeepL автоматически выбирает Free endpoint для ключей с суффиксом `:fx`. Перед переводом доступна отдельная проверка подключения.
...
- создавать черновой перевод описания через LibreTranslate, DeepL или Google Cloud Translation с ручным подтверждением результата;
